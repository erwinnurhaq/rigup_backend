...
const { transporter, transactionWaitingPaymentEmail, transactionPaymentSuccessEmail } = require('../config/mailer');
...
module.exports = {
    ...
    getAllTransactions: async (req, res) => {
        try {
            ...
            let query = `SELECT COUNT(id) as sum FROM transactions WHERE paidStatus != 3 OR deliveredStatus != 3`
            const allCount = await dbquery(query)
            query = `SELECT t.*, u.username, u.email FROM transactions t
                        join users u on u.id = t.userId
                        WHERE t.deliveredStatus < 2
                        order by ${order}
